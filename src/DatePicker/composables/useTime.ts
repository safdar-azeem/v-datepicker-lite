...
export function useTime(
	...
	const generateTimeOptions = () => {
		...
		for (
			let period = 0;
			period < (is12HourFormat.value ? 2 : 1);
			period++
		) {
			const periodSuffix = is12HourFormat.value
				? period === 0
					? ' AM'
					: ' PM'
				: ''
			...
			for (
				...
				for (let m = 0; m < 60; m += timeInterval) {
					...
					options.push({
						...
						value: `${String(hourValue).padStart(2, '0')}:${String(
							m
						).padStart(2, '0')}`,
						hour: hourValue,
						minute: m,
	...
	const scrollToSelected = (timeListRef: HTMLDivElement | null) => {
		...
		const selectedIndex = timeOptions.value.findIndex(
